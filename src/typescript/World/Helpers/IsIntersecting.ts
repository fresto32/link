...
export default function IsIntersecting(
  ...
  return object.children.some(c => {
    if (c instanceof THREE.Mesh) {
      return pointsOfIntersection(plane, c);
    } else if (c instanceof THREE.Group) {
      return c.children.some(v =>
        IsIntersecting(plane, v as THREE.Group | THREE.Mesh)
      );
    } else {
      return false;
    }
...
function pointsOfIntersection(plane: THREE.Mesh, object: THREE.Mesh): boolean {
  ...
  function setMathPlane(plane: THREE.Mesh): THREE.Plane {
    ...
    if (plane.geometry instanceof THREE.BufferGeometry) {
      ...
      if (plane.geometry.attributes.index !== undefined) {
        if (
          plane.geometry.attributes.position instanceof
        ...
        plane.localToWorld(
          planePointA.fromBufferAttribute(plane.geometry.attributes.position, 0)
        );
        plane.localToWorld(
          planePointB.fromBufferAttribute(plane.geometry.attributes.position, 1)
        );
        plane.localToWorld(
          planePointC.fromBufferAttribute(plane.geometry.attributes.position, 2)
        );

        // Create a plane from the world localised points...
        mathPlane.setFromCoplanarPoints(planePointA, planePointB, planePointC);
      } // buffer geometry must be non-indexed
      else {
  ...
  function processBufferFaces(
    object: THREE.Mesh,
    geometry: THREE.BufferGeometry,
    mathPlane: THREE.Plane
  ) {
    if (
      geometry.attributes.position instanceof THREE.InterleavedBufferAttribute
    ) {
      console.log(
        'Interleaved Buffer Geometry sent to processBufferFaces(...)'
      );
      return false;
    } else if (geometry.index === null) {
      console.log(
        'Geometry with no index array sent to processBufferFaces(...)'
      );
      return false;
    }
